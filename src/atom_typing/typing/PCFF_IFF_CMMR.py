...
Atom typing rules for PCFF-IFF with user-defined assumed types
...
def nta(mm, basename, ff_name):
    ...
    ################################################
    # Set PCFF-IFF specific informations and flags #
    ################################################
    # Log supported types
    supported_types = {'Carbon':      ['ct', 'c+', 'cr', 'c-', 'c5', 'cs', 'cp', 'cg1 (Q)', 'c_0',
                                       'c_1', 'c_2', 'cz', 'ci', 'c=', 'c=1', 'c=2', 'c3h',
                                       'c3m', 'c4h', 'c4m', 'c_a', 'cg', 'co', 'coh', 'ce1 (Q)',
                                       'c1', 'c2', 'c3', 'c4o', 'c'],
                       ...
                       'Nitrogen':    ['nt', 'ni', 'np', 'n=', 'n=1', 'n=2', 'n1', 'n2', 'n_2', 'nho',
                                       'ni', 'npc', 'nh', 'n3n', 'n3m', 'n4n', 'n4m', 'nn', 'na', 'n',
                                       'nh+', 'n4'],
                       
                       'Sulfer':      ["s'", 's-', 's3e', 's4e', 'sp', 'sc', 'sh', 's1', 's', 'sf', 's_m'],
                       
                       'Silicone':    ['sio', 'sc4', 'sc1', 'si'],
    ...
    supported_types = tf.update_supported_types(supported_types, element='Carbon', atomtype='cg1 (Q)', flag=use_graphene_types)
    supported_types = tf.update_supported_types(supported_types, element='Carbon', atomtype='ce1 (Q)', flag=use_peo_poly_types)
    supported_types = tf.update_supported_types(supported_types, element='Hydrogen', atomtype='cge (Q)', flag=use_graphene_types)
    ...
    ##########################################################################
    # Set Assumed PCFF-IFF atomtypes if code fails to find correct atom-type #
    ##########################################################################
    ...
    for i in mm.atoms:
        ...
        if element == 'C' and nb <= 2:
            ...
            #----------------------------------------------------------------------------#
            # cg1     10.011150    C           3        sp2 hybridized carbon atom in graphene sheet
            # If an edge atom in graphene and use_graphene_types is True set as graphene type
            if ring == 6 and 6 in rings1 and rings3.count(6) >= 3 and use_graphene_types:
                atom.nta_type = 'cg1'; tally['found'] += 1;
                atom.nta_info = 'Edge-atom in graphitic sheet (element:C, ring:6, nb:2)'
                atom.charge = 0.300000 # reset C-element charge for IFF if graphene atom-type
        ...
        ######################################################################################
            ...
            # cg1     10.011150    C           3        sp2 hybridized carbon atom in graphene sheet
            # Check for 100% match of 1st-neigh elements in carbon 6-member rings (If pi-electrons are attached they will be handled eleswhere)
            elif ring == 6 and tf.percent_neigh(atom.neighbor_info[1], element='C', ring=6, nb=False) == 100 and use_graphene_types:
                atom.nta_type = 'cg1'; tally['found'] += 1;
                atom.nta_info = 'Correctly found'
                atom.charge = 0.300000 # reset C-element charge for IFF if graphene atom-type
            ...
            # cp       12.01115      C          3        sp2 aromatic carbon
            elif ring == 6 and not use_graphene_types and tf.check_aromaticity(i, mm.atoms):
                atom.nta_type = 'cp'; tally['found'] += 1;
                atom.nta_info = 'Correctly found'
            elif ring == 6 and rings1.count(6) <= 2 and tf.check_aromaticity(i, mm.atoms):
                atom.nta_type = 'cp'; tally['found'] += 1;
                atom.nta_info = 'Correctly found'
        ...
        ######################################################################################
            ...
            elif ring == 0  and tf.check_peo_topo(atom, element_type='C') and use_peo_poly_types:              
        ...
        ##########################################################
        # IFF's weired pi-electrons for cg1 typeing makes nb > 4 #
        ##########################################################
        elif element == 'C' and nb > 4:
            # cg1     10.011150    C           3        sp2 hybridized carbon atom in graphene sheet
            if ring == 6 and 6 in rings1 and rings3.count(6) >= 3 and use_graphene_types:
            ...
            # cp       12.01115      C          3        sp2 aromatic carbon
            elif ring == 6 and not use_graphene_types:
                atom.nta_type = 'cp'; tally['found'] += 1;
                atom.nta_info = 'Correctly found'
        ...
        ####################################################################################
            ...
            # If pi-electrons are on molecule the 1st neighbor carbon atom will have 5 bonded atoms
            elif elements1[0] == 'C' and nbs1[0] == 5 and rings1[0] == 6 and use_graphene_types:
            ...
            elif elements1[0] == 'O' and 'Si' in elements2 or 'Mg' in elements2 or 'Al' in elements2:
                atom.nta_type = 'hoy'; tally['found'] += 1;
                atom.nta_info = 'Correctly found'
            ...
            elif elements1[0] == 'S' and formula == 'C3-H7-N1-O1':
                atom.nta_type = 'hdm'; tally['found'] += 1;
                atom.nta_info = 'Correctly found'
        ...
        ###########################################################################################
            ...
            # oc24    15.99940      O         2        Oxygen atom on silica surface and clay edge (-0.675 in Si-OH, -0.9 in SiO-) 
                ...
                atom.nta_type = 'oc23'; tally['found'] += 1;
                atom.nta_info = 'Correctly found'
        ...
        #############################################################################################
            ...
            if ring == 5 and tf.count_neigh(atom.neighbor_info[1], element='C', ring=5, nb=3) >= 2 and formula == 'C3-H4-N2':
        ...
        ############################################################################################
            ...
            elif ring == 0 and elements1.count('O') == 4:          
                atom.nta_type = 'sc4'; tally['found'] += 1;
                atom.nta_info = 'Correctly found'
                
            # sc1     28.08600     Si          4         Silicon atom in silicate ion in Ca3SiO5 (+1.0)
            elif formula == 'Ca1-O5-Si1':
                atom.nta_type = 'sc1'; tally['found'] += 1;
                atom.nta_info = 'Correctly found'

...
def merge(topofile, mass_map, ff_class, log):
    ...
    for i in m.atoms:
        ...
        try:
            ...
            if atom.comment == 'N/A': atom.comment = element
    ...
    for i in m.masses:
        ...
        if mass.type == 'N/A': mass.type = mass.element
    ...
    r0_range = [0.0, 5.0] # Set min/max r0s to check that r0 is at index 0
    for i in m.bond_coeffs:
        coeffs = m.bond_coeffs[i].coeffs
        if ff_class in [1, '1'] and len(coeffs) != 2: log.error(f'ERROR read in {topofile} Bond Coeff {i} is not harmonic (class1)')
        if ff_class in [2, '2'] and len(coeffs) != 4: log.error(f'ERROR read in {topofile} Bond Coeff {i} is not class2')
        
        # Get r0 based on harmonic vs class2
        try:
            if ff_class in [1, '1']: k, r0 = coeffs
            if ff_class in [2, '2']: r0, k2, k3, k4 = coeffs

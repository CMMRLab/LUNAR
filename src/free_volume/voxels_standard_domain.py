...
class generate:
    def __init__(self, m, max_voxel_size, boundary, vdw_radius, probe_diameter, vdw_method, run_mode, log):  
        ...
        self.nxx = math.ceil(self.lx/self.domain_size)
        self.nyy = math.ceil(self.ly/self.domain_size)
        self.nzz = math.ceil(self.lz/self.domain_size)
        dx = self.lx/self.nxx; dy = self.ly/self.nyy; dz = self.lz/self.nzz;
        halfdx = dx/2; halfdy = dy/2; halfdz = dz/2;
        xadd = halfdx + self.xlo; yadd = halfdy + self.ylo; zadd = halfdz + self.zlo;
        ID = 0; self.domain = {} # { domainID : (xlo, xhi, ylo, yhi, zlo, zhi, xc, yc, zc) }
        log.out('\n\nFinding domain to use cell linked list algorithm ...')
        for nx in tqdm(range(self.nxx)):
            for ny in range(self.nyy):
                for nz in range(self.nzz):
                    ID += 1
                    xc = nx*dx + xadd
                    yc = ny*dy + yadd
                    zc = nz*dz + zadd
                    ...
                    zlo = zc - halfdz
                    zhi = zc + halfdz
                    self.domain[ID] = (xlo, xhi, ylo, yhi, zlo, zhi, xc, yc, zc)
        
        # Find domain connectivity (graph)
        self.domain_graph = {ID:set() for ID in self.domain} # { ID : set(bonded IDs) }
        log.out('\n\nFinding cell linked graph ...')
        for id1 in tqdm(self.domain):
            d1 = self.domain[id1]
            x1 = d1[6]; y1 = d1[7]; z1 = d1[8];
            for id2 in self.domain:
                if id1 == id2: continue
                d2 = self.domain[id2]
                x2 = d2[6]; y2 = d2[7]; z2 = d2[8];
                for ixlx, iyly, izlz in self.scaled_images:
                    x1i = x1+ixlx; y1i = y1+iyly; z1i = z1+izlz;
                    if abs(x1i - x2) > self.domain_size: continue
                    elif abs(y1i - y2) > self.domain_size: continue
                    elif abs(z1i - z2) > self.domain_size: continue
                    else:
                        self.domain_graph[id1].add(id2)
                        self.domain_graph[id2].add(id1)
                        break
        ...
        for i in m.atoms:
            ...
            for j in self.domain:
                xlo, xhi, ylo, yhi, zlo, zhi, xc, yc, zc = self.domain[j]
                if xlo <= x <= xhi and ylo <= y <= yhi and zlo <= z <= zhi:
                    atom.domainID = j; break;
        
        #-----------------------#
        # Generate voxels step2 #
        #-----------------------#        
        # Generate voxels 
        ...
        self.voxel_domains = {i:set() for i in self.domain} # { domainID : voxelIDs in domain }
        xadd = self.halfdx + self.xlo; yadd = self.halfdy + self.ylo; zadd = self.halfdz + self.zlo;
        domain_guess = 1
        log.out('\n\nGenerating voxels ...')
        for nx in tqdm(range(self.nx)):
            for ny in range(self.ny):
                for nz in range(self.nz):
                    ...
                    # Use previous voxel domain to guess current voxel domain (sped-up)
                    d = self.domain[domain_guess]
                    if d[0] <= xpos <= d[1] and d[2] <= ypos <= d[3] and d[4] <= zpos <= d[5]:
                        domainID = domain_guess
                        self.voxel_domains[domainID].add(ID);
                    else:
                        for domainID in self.domain:
                            xlo, xhi, ylo, yhi, zlo, zhi, xc, yc, zc = self.domain[domainID]
                            if xlo <= xpos <= xhi and ylo <= ypos <= yhi and zlo <= zpos <= zhi: 
                                self.voxel_domains[domainID].add(ID); domain_guess = domainID; break;
...
class analyze:
    def __init__(self, m, v, vdw_radius, boundary, max_voxel_size, compute_free_volume_distributions, mass_map, run_mode, files2write, probe_diameter, vdw_method, log):         
        ...
        freeIDs = {i:True for i in v.voxelIDs}; self.voxel_atomIDs = {}; self.pbc_count = 0; self.npossible_pbc = 0; 
        for i in tqdm(m.atoms):
            atom = m.atoms[i]; periodic_switch = 0;
            ...
            atom_domains = [atom.domainID] + list(v.domain_graph[atom.domainID])
            for domainID in atom_domains:
                voxelIDs = v.voxel_domains[domainID]
                for ID in voxelIDs:
                    if not freeIDs[ID]: continue
                            ...
                            for x1i, y1i, z1i in periodic_postions:
                                if abs(x1i - x2) > vdw_radii: continue
                                    ...
                                    if distance_ppp < vdw_radii:
                                        ...
                                        self.voxel_atomIDs[ID] = (x2, y2, z2, TypeID)
                                        freeIDs[ID] = False; self.pbc_count += 1;
        ...
        self.voxel_freeIDs = {ID:v.voxelIDs[ID] for ID in freeIDs if freeIDs[ID]}

...
def generate_nta(m, frc, reset_charges, log):
    ...
    # Find nta atom types for reaxFF work and find x, y, z pos for box positioning
    ...
    if frc.extend_elements:
        # print warning
        log.warn(f'\nWARNING reaxFF conversion is using {frc.filename} with #extend_elements Add: {" ".join(frc.extend_elements)}. This option is meant to make')
    ...
    if not check_images(system_periodicity):
        ...
        if frc.zero_dim_buffer == 0: 
            buffer = 0.5; log.out(f'ReaxFF zero_dim_buffer was zero, resulting in a {buffer} angstrom over ride')

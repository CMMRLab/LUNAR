...
Finally this script will show how to access the data used
during a log_analysis run and how to plot that data from
within this script.
"""
#########################################
...
if __name__ == "__main__": 
    ...
    #       cases.
    #
    # For this example, we will intialize the logging object with the defaults, by not specifying
    # any parameters. The defaults are: level='production', print2console=True, write2log=True
    log = io_functions.LUNAR_logger()
    log.configure(print2console=False, write2log=False)
    ...
    # Get desired data and Generate a plot from the data
    xraw = analyzed.outputs['Raw-data']['xdata']
    yraw = analyzed.outputs['Raw-data']['ydata']
    xavg = analyzed.outputs['Density average']['xdata']
    yavg = analyzed.outputs['Density average']['ydata']
    
    fig, ax = plt.subplots()
    ax.plot(xraw, yraw, label='Raw data used in analysis')
    ax.plot(xavg, yavg, label='Data used in averaging')

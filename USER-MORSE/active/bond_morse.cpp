...
void BondMorse::compute(int eflag, int vflag)
{
  ...
  for (n = 0; n < nbondlist; n++) {
    ...
    if (eflag) ebond = d0[type] * (1 - ralpha) * (1 - ralpha) - offset[type];
...
void BondMorse::allocate()
{
  ...
  memory->create(rcut, np1, "bond:rcut");
...
void BondMorse::coeff(int narg, char **arg)
{
  ...
  double rcut_one = narg > 4 ? utils::numeric(FLERR, arg[4], false, lmp) : 0;
  double offset_one = narg > 5 ? utils::numeric(FLERR, arg[5], false, lmp) : 0;
  ...
  for (int i = ilo; i <= ihi; i++) {
    ...
    alpha[i] = alpha_one;
    r0[i] = r0_one;
    if ((rcut_one > 0.0) && (offset_one <= 0.0)) {
      double fforce;
      offset_one = single(i, rcut_one*rcut_one, i, i, fforce);
    }
    rcut[i] = rcut_one;
    offset[i] = offset_one;
    setflag[i] = 1;

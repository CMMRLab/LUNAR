...
PairLJ::PairLJ(LAMMPS *lmp) : Pair(lmp) {}
...
void PairLJ::compute(int eflag, int vflag)
{
...
void PairLJ::settings(int narg, char **arg)
{
...
void PairLJ::coeff(int narg, char **arg)
{
  if (narg != 4 && narg != 5)
...
double PairLJ::init_one(int i, int j)
{
  if (setflag[i][j] == 0) error->all(FLERR,"All pair coeffs are not set");

  cutsq[i][j] = cutsq[j][i] = cut[i][j] * cut[i][j];

  lj1[i][j] = lj1[j][i] = 12.0 * aparm[i][j];
  lj2[i][j] = lj2[j][i] =  6.0 * bparm[i][j];
  lj3[i][j] = lj3[j][i] = aparm[i][j];
  lj4[i][j] = lj4[j][i] = bparm[i][j];
...
void PairLJ::write_restart(FILE *fp)
{
...
void PairLJ::read_restart(FILE *fp)
{
...
void PairLJ::write_restart_settings(FILE *fp)
{
...
void PairLJ::read_restart_settings(FILE *fp)
{
  int me = comm->me;
  if (me == 0) {
...
void PairLJ::write_data(FILE *fp)
{
...
void PairLJ::write_data_all(FILE *fp)
{
  for (int i = 1; i <= atom->ntypes; i++)
    for (int j = i; j <= atom->ntypes; j++)
      fprintf(fp,"%d %d %g %g %g\n", i,j,aparm[i][j],bparm[i][j],cut[i][j]);
}

/* ---------------------------------------------------------------------- */

double PairLJ::single(int i, int j, int itype, int jtype,
                             double rsq,
...
void *PairLJ::extract(const char *str, int &dim)
{
  dim = 2;
